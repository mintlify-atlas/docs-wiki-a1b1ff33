...
public class Post : FullAuditedAggregateRoot<Guid>
{
    ...
    [NotNull]
    public virtual string CoverImage { get; set; }       // cover image URL or path
...
The constructor signature is `Post(Guid id, Guid blogId, string title, string coverImage, string url)`. All three fields `Title`, `CoverImage`, and `Url` are non-nullable and validated with `Check.NotNullOrWhiteSpace`.
...
{
    public virtual Guid PostId { get; protected set; }          // references Post.Id
    public virtual Guid? RepliedCommentId { get; protected set; }
    public virtual string Text { get; protected set; }
}
```

Unlike CmsKit's `Comment` (which is polymorphic across entity types via `EntityType`/`EntityId`), the Blogging `Comment` is hardcoded to `PostId` (not `BlogPostId`) referencing the `Post` aggregate.
...
public interface IPostRepository : IBasicRepository<Post, Guid>
{
    ...
    Task<bool> IsPostUrlInUseAsync(
        Guid blogId, string url, Guid? excludingPostId = null,
        CancellationToken cancellationToken = default);

    Task<Post> GetPostByUrl(Guid blogId, string url, CancellationToken cancellationToken = default);

    Task<List<Post>> GetOrderedList(
        Guid blogId, bool descending = false, CancellationToken cancellationToken = default);
...
| **Comment model** | `PostId` (hardcoded to Post) | Polymorphic via `EntityType`/`EntityId` |
