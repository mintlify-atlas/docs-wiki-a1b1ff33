...
Represents an OAuth2 / OIDC client registration. Stored as a `FullAuditedAggregateRoot<Guid>` in namespace `Volo.Abp.OpenIddict.Applications`:
...
public class OpenIddictApplication : FullAuditedAggregateRoot<Guid>
{
    ...
    public virtual string Properties { get; set; }     // JSON: additional properties
    ...
    public virtual string ClientUri { get; set; }      // link to further info about client
    public virtual string LogoUri { get; set; }        // URI to client logo
}
```
...
Defines an OAuth2 scope that can be granted to clients (namespace `Volo.Abp.OpenIddict.Scopes`):
...
Records a user's consent grant, linking a subject (user ID) to an application and a set of scopes (namespace `Volo.Abp.OpenIddict.Authorizations`):
...
Stores issued tokens (access tokens, refresh tokens, authorization codes, device codes) in namespace `Volo.Abp.OpenIddict.Tokens`:
...
Expired and revoked tokens accumulate over time. `TokenCleanupBackgroundWorker` runs on a configurable interval and calls `TokenCleanupService`. The actual `TokenCleanupOptions` class (from `Volo.Abp.OpenIddict.Tokens`) contains:
...
public class TokenCleanupOptions
{
    ...
    /// <summary>Default: 3,600,000 ms (1 hour).</summary>
    public int CleanupPeriod { get; set; } = 3_600_000;

    /// <summary>Disable pruning of old authorization records.</summary>
    public bool DisableAuthorizationPruning { get; set; }

    /// <summary>Disable pruning of old token records.</summary>
    public bool DisableTokenPruning { get; set; }

    /// <summary>
    /// Minimum lifespan authorizations must have to be pruned.
    /// Default: 14 days; cannot be less than 10 minutes.
    /// </summary>
    public TimeSpan MinimumAuthorizationLifespan { get; set; } = TimeSpan.FromDays(14);

    /// <summary>
    /// Minimum lifespan tokens must have to be pruned.
    /// Default: 14 days; cannot be less than 10 minutes.
    /// </summary>
...
<Steps>
  ...
  <Step title="Re-seed clients">
    Update data seed contributors to use `AbpApplicationDescriptor` / `OpenIddictDataSeedContributorBase` instead of IdentityServer's `IdentityServerDataSeedContributor`.
  </Step>
