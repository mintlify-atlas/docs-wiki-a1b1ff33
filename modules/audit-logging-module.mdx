...
<CardGroup cols={2}>
  ...
  <Card title="Domain" icon="cube">
    `Volo.Abp.AuditLogging.Domain` — `AuditLog`, `EntityChange`, `EntityPropertyChange`, `AuditLogAction` entities; `IAuditLogRepository`, `AuditingStore`, `AuditLogInfoToAuditLogConverter`, `IAuditLogExcelFileRepository`
  </Card>
  <Card title="EntityFrameworkCore" icon="database">
    `Volo.Abp.AuditLogging.EntityFrameworkCore` — `AbpAuditLoggingDbContext`, EF Core `IAuditLogRepository` and `IAuditLogExcelFileRepository` implementations, migration history table configuration
  </Card>
...
Field lengths are capped by constants in `AuditLogConsts` (e.g., `MaxUrlLength = 256`, `MaxUserNameLength = 256`, `MaxBrowserInfoLength = 512`) using `Truncate()` extensions in the constructor.
...
public class AuditingStore : IAuditingStore, ITransientDependency
{
    public virtual async Task SaveAsync(AuditLogInfo auditInfo)
    {
        ...
        {
            Logger.LogWarning("Could not save the audit log object: " + auditInfo);
...
| Table | Key columns |
|---|---|
| `AbpAuditLogs` | `Id`, `TenantId`, `UserId`, `ExecutionTime`, `HttpStatusCode`, `Url`, `CorrelationId` |
