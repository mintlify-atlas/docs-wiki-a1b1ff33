...
public class AbpAspNetCoreComponentsWebAssemblyModule : AbpModule
{
    ...
    {
        // 1. Fetch application configuration (uses IClientScopeServiceProviderAccessor)
...
- **Accept-Language header** — reads the selected language from `localStorage.getItem("Abp.SelectedLanguage")` via JS interop
- **AntiForgery header** — reads the `XSRF-TOKEN` cookie and sends it as `RequestVerificationToken` on non-safe HTTP methods (not GET/HEAD/OPTIONS/TRACE), but only for same-origin requests
- **Timezone header** — adds `Abp-Timezone` from `ICurrentTimezoneProvider.TimeZone`
- **UI progress** — calls `IUiPageProgressService.Go()` to show/hide the page progress bar around each HTTP request
- **Streaming** — sets `SetBrowserRequestStreamingEnabled(true)` for HTTPS requests

<Note>
The handler does **not** add an `Authorization` header. Bearer token injection is handled separately by the OIDC client library and its own HTTP handler registered by `@abp/ng.oauth` on the Angular side, or by a dedicated `AbpBearerTokenHttpClientHandler` for .NET clients.
</Note>
