...
`IWebRequestResources` is a request-scoped service that deduplicates resource URLs across multiple bundle tag helpers on the same page. If `abp-script-bundle` and `abp-style-bundle` both try to add the same file (via base bundle inheritance), `TryAdd` ensures it appears only once.
