...
- **`IsInherited`** — when `true` (the default), a setting value can be inherited from a higher scope if no lower-scope override exists. Note: full multi-scope inheritance depends on the provider implementations.
