...
<CardGroup cols={2}>
  <Card title="IAbpAuthorizationService" icon="shield-check">
    Extends both `IAuthorizationService` and `IServiceProviderAccessor`. ABP replaces the default ASP.NET Core implementation with `AbpAuthorizationService`, which sources the principal from `ICurrentPrincipalAccessor` via the `CurrentPrincipal` property so it works correctly outside an HTTP context.
  </Card>
  ...
  <Card title="PermissionDefinition" icon="code">
    Metadata describing a single permission: unique `Name`, `DisplayName`, `MultiTenancySide`, hierarchical `Children`, allowed `Providers`, `StateCheckers`, and arbitrary `Properties`. Also carries optional `ResourceName` and `ManagementPermissionName` for resource-based permissions.
  </Card>
...
classDiagram
    ...
    IServiceProviderAccessor <|-- IAbpAuthorizationService
    ...
    ICanAddChildPermission
{
    ...
    public string? ResourceName { get; set; }          // for resource-based permissions
...
`StateCheckers` allow attaching `ISimpleStateChecker<PermissionDefinition>` implementations that can dynamically enable/disable a permission at evaluation time based on application state (e.g., feature flags).

## Permission Value Providers

The permission evaluator iterates `IPermissionValueProviderManager.Providers` **in reverse order** (highest-priority provider wins). ABP registers three built-in providers out of the box:
