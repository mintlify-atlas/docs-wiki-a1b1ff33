...
The resolution algorithm mirrors the settings chain: providers are reversed (highest priority last in registration order, first in iteration) and queried until a non-null value is found. The typical provider order registered by the commercial edition is:
...
The `GlobalFeatureNameAttribute` must be present on every global feature class — it supplies the string key used in `EnabledFeatures`. Calling `GlobalFeatureManager.Instance.Enable(typeof(TFeature))` without the attribute throws `AbpException`.
