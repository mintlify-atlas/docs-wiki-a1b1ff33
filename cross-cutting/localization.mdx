...
<CardGroup cols={2}>
  <Card title="LocalizationResource" icon="book">
    A named container for contributors and base resources. Constructed from a marker type class decorated with `[LocalizationResourceName]`. Inherits from `LocalizationResourceBase`.
  </Card>
    ...
    LocalizationResourceBase resource)
{
    ...
    // Build base localizers recursively (lockCache: false to avoid deadlock)
...
`AbpStringLocalizerFactory` also consults `IExternalLocalizationStore` when looking up resources by name. This enables localization resources defined outside the application assembly (e.g., loaded from a plugin or remote endpoint) to participate in the same resolution pipeline without being registered in `AbpLocalizationOptions.Resources` at compile time.
