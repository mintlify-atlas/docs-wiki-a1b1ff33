...
3. `auditingHelper.ShouldSaveAudit(method)` — respects `[Audited]` / `[DisableAuditing]` attributes and class-level defaults

`ShouldWriteAuditLog` then applies post-execution filters:
...
| `!IsEnabledForGetRequests && GET/HEAD` | Skip |
