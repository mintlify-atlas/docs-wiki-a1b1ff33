...
### `IRepository` — the root marker

The root `IRepository` interface (non-generic) defines only three members that every repository must expose:
...
`ProviderName` identifies the underlying ORM (`"EntityFrameworkCore"`, `"MongoDB"`, etc.). `IsChangeTrackingEnabled` can be `null` (use the default policy), `true`, or `false`. `EntityName` is an optional label used for tracing and logging.
...
<Note>
`IReadOnlyRepository` also contains two `WithDetails()` overloads (without `Async`) that are marked `[Obsolete]`. Always use `WithDetailsAsync()` in new code.
</Note>
...
The fully keyed `IRepository<TEntity, TKey>` composes `IRepository<TEntity>`, `IReadOnlyRepository<TEntity, TKey>`, and `IBasicRepository<TEntity, TKey>` — giving you every method in the hierarchy.
...
## `BasicRepositoryBase<TEntity>` internals

`BasicRepositoryBase<TEntity>` is the lowest shared implementation layer. It implements `IBasicRepository<TEntity>` and resolves all infrastructure services lazily through `IAbpLazyServiceProvider`:
...
It also provides `ShouldTrackingEntityChange()` — a method that respects the `IsChangeTrackingEnabled` flag first, then the `IEntityChangeTrackingProvider` (which checks `[EnableEntityChangeTracking]` / `[DisableEntityChangeTracking]` attributes on the call site), then defaults to `true`.

## `RepositoryBase<TEntity>` internals

`RepositoryBase<TEntity>` extends `BasicRepositoryBase<TEntity>` and adds the queryable (`IRepository<TEntity>`) contract:
...
`FindAsync`, `DeleteAsync(predicate)`, and `DeleteDirectAsync` are declared `abstract` here — concrete providers implement them. `GetAsync(predicate)` is implemented by delegating to the abstract `FindAsync`.

There is also `RepositoryBase<TEntity, TKey>` which extends `RepositoryBase<TEntity>` and adds key-based `GetAsync(TKey)`, `FindAsync(TKey)`, `DeleteAsync(TKey)`, and `DeleteManyAsync(IEnumerable<TKey>)`.
...
The `SaveChangesAsync` implementation in `BasicRepositoryBase` delegates to `UnitOfWorkManager.Current?.SaveChangesAsync()` — if there is no active Unit of Work it is a no-op.
