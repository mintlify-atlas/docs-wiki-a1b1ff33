...
| `rc.*` | `Prerelease` | NuGet.org `Volo.Abp.Cli` (pre-release) |
...
{
    options.Generators["JS"]     = typeof(JavaScriptServiceProxyGenerator);
    options.Generators["NG"]     = typeof(AngularServiceProxyGenerator);
    options.Generators["CSHARP"] = typeof(CSharpServiceProxyGenerator);
});
```
...
<Note>
The `HideFromCommandList` attribute (in the `Internal` sub-namespace) suppresses a command from the `help` output while still registering it normally. `RecreateInitialMigrationCommand` uses this to hide internal tooling commands from end users.
</Note>
