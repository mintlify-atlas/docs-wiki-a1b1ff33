...
<Note>
The `MyCompanyName.MyProjectName.MauiBlazor` project and the `/react-native/` folder (mobile) are supported by the pipeline code and present in the Pro/commercial template variants. They are removed by the pipeline unless `--ui maui-blazor` or `--mobile react-native` is passed respectively.
</Note>
