...
`QueueAsync` enqueues a `BackgroundEmailSendingJobArgs` via `IBackgroundJobManager`. If no background job manager is available (`IsAvailable()` returns false), it falls back to synchronous delivery:
...
The `TenantId` is captured from `ICurrentTenant` at enqueue time, ensuring the email is sent in the correct tenant context when the background job runs.
...
<Warning>
`SmtpEmailSender` uses `System.Net.Mail.SmtpClient`, which Microsoft has marked as legacy and lacking modern protocol support (OAuth, STARTTLS negotiation). The framework itself logs a warning. Use the MailKit integration (`Volo.Abp.MailKit`) for production systems.
</Warning>
...
`NullEmailSender` extends `EmailSenderBase` with a no-op `SendEmailAsync`. It logs a `LogWarning` indicating it is active, then logs the email details at `LogDebug`:
