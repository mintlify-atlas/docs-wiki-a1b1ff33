...
`tryDefaults = true` tells the provider to fall back to the `DefaultCultureName` or the neutral template if the requested culture's content is unavailable. Content can come from Virtual File System, database, or any custom `ITemplateContentContributor`.
...
`WithProperty(key, value)` stores arbitrary metadata that provider-specific code can read. `WithRenderEngine` sets `RenderEngine`, which `ITemplateRenderer` uses to dispatch to the correct rendering engine.
...
The three-phase lifecycle (Pre → Define → Post) allows modules to define templates and then allows other modules to customize or extend them in `PostDefine`. ABP discovers all `ITemplateDefinitionProvider` implementations via DI during module initialization.
...
The `IsSandboxed = true` means only public properties (not methods) on injected objects are accessible from templates, enforced via the `MemberFilter`:
...
<Steps>
  ...
  <Step title="Fetch raw content">
    `GetContentOrNullAsync(templateDefinition)` calls `ITemplateContentProvider` with the current culture. Falls back to `DefaultCultureName` if the culture-specific content is absent.
  </Step>
...
```mermaid
flowchart TD
    APP[Application Code] -->|RenderAsync| TR[ITemplateRenderer]
    TR --> TDM[ITemplateDefinitionManager]
    TDM --> TDP[ITemplateDefinitionProvider\nregistrations]
    TR --> TCP[ITemplateContentProvider]
    TCP --> VFS[Virtual File System\ncontent store]
    TCP --> DB[Database\ncontent store]
    TR -->|EngineName lookup| SE[ScribanTemplateRenderingEngine]
    TR -->|EngineName lookup| RE[RazorTemplateRenderingEngine]
    SE --> SCRIBAN[Scriban.Template.Parse\n+ RenderAsync]
    SE --> LOC[ScribanTemplateLocalizer\nL variable]
    SE -->|Layout defined| TR
```
